...
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    EdgeSim3ProjectXYZ(Sophus::SE3d Tc = Sophus::SE3d());
    virtual bool read(std::istream& is);
    virtual bool write(std::ostream& os) const;
    ...
    void computeError()
    {
        ...
        Eigen::Vector2d obs(_measurement);
        _error = obs-v1->pCamera1->project(mTc * (v1->estimate().map(v2->estimate())));
    }
    Sophus::SE3d mTc;
    // virtual void linearizeOplus();
...
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    EdgeInverseSim3ProjectXYZ(Sophus::SE3d Tc = Sophus::SE3d());
    virtual bool read(std::istream& is);
    virtual bool write(std::ostream& os) const;
    ...
    void computeError()
    {
        ...
        Eigen::Vector2d obs(_measurement);
        _error = obs-v1->pCamera2->project(mTc * (v1->estimate().inverse().map(v2->estimate())));
    }
    ...
    class EdgeSim3XYZ : public  g2o::BaseBinaryEdge<3, Eigen::Vector3d,  g2o::VertexSBAPointXYZ, VertexSim3Expmap>
    {
    public:
        EIGEN_MAKE_ALIGNED_OPERATOR_NEW
        EdgeSim3XYZ();
