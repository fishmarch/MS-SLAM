...
namespace ORB_SLAM3
{
            ...
            if(pMP->mbTrackInView)
            {
                ...
                if(!vIndices.empty()){
                    ...
                    for(vector<size_t>::const_iterator vit=vIndices.begin(), vend=vIndices.end(); vit!=vend; vit++)
                    {
                        ...
                        if(F.mvpMapPoints[idx] && !pMP->mbSparsified)
    ...
    int ORBmatcher::SearchByBoW(shared_ptr<KeyFrame>  pKF,Frame &F, vector<shared_ptr<MapPoint>> &vpMapPointMatches)
    {
        ...
        const DBoW2::FeatureVector &vFeatVecKF = pKF->GetFeatureVector();
                ...
                for(size_t iKF=0; iKF<vIndicesKF.size(); iKF++)
                {
                    ...
                    if(bestDist1<=TH_LOW)
                    {
                        if(static_cast<float>(bestDist1)<mfNNratio*static_cast<float>(bestDist2))
                        {
                            ...
                            const cv::KeyPoint &kp = pKF->GetKeyPoint(realIdxKF);
                                       ...
                                       vector<shared_ptr<MapPoint>> &vpMatched, int th, float ratioHamming)
    {
        ...
        for(int iMP=0, iendMP=vpPoints.size(); iMP<iendMP; iMP++)
        {
            ...
            for(vector<size_t>::const_iterator vit=vIndices.begin(), vend=vIndices.end(); vit!=vend; vit++)
            {
                ...
                const int &kpLevel= pKF->GetKeyUn(idx).octave;
    ...
    int ORBmatcher::SearchByProjectionLoop(shared_ptr<KeyFrame>  pKF, Sophus::Sim3f &Scw, const vector<shared_ptr<MapPoint>> &vpPoints,
                                       vector<shared_ptr<MapPoint>> &vpMatched, std::vector<shared_ptr<KeyFrame>> &vpMatchedKF, int th, float ratioHamming)
    {
        // Get Calibration Parameters for later projection
        const float &fx = pKF->fx;
        const float &fy = pKF->fy;
        ...
        const vector<shared_ptr<MapPoint>> vpMapPointsToMatch = pKF->GetMapPointMatches();
        Sophus::SE3f Tcw = Sophus::SE3f(Scw.rotationMatrix(),Scw.translation()/Scw.scale());
        Eigen::Vector3f Ow = Tcw.inverse().translation();
        ...
        for(int iMP=0, iendMP=vpPoints.size(); iMP<iendMP; iMP++)
        {
            ...
            for(vector<size_t>::const_iterator vit=vIndices.begin(), vend=vIndices.end(); vit!=vend; vit++)
            {
                ...
                if(!vpMapPointsToMatch[idx] || vpMapPointsToMatch[idx]->isBad())
                    continue;
            ...
            if(bestDist<=TH_LOW*ratioHamming)
            {
                vpMatched[iMP]=vpMapPointsToMatch[bestIdx];
                vpMatchedKF[iMP]=pKF;
                nmatches++;
            }
    ...
    {
        const vector<cv::KeyPoint> &vKeysUn1 = pKF1->GetAllKeyUn();
        const DBoW2::FeatureVector &vFeatVec1 = pKF1->GetFeatureVector();
...
//        const std::vector<cv::Mat> &Descriptors1 = pKF1->mDescriptors;
        ...
        const vector<cv::KeyPoint> &vKeysUn2 = pKF2->GetAllKeyUn();
        const DBoW2::FeatureVector &vFeatVec2 = pKF2->GetFeatureVector();
...
//        const std::vector<cv::Mat> &Descriptors2 = pKF2->mDescriptors;
        ...
        while(f1it != f1end && f2it != f2end)
        {
            if(f1it->first == f2it->first)
            {
                for(size_t i1=0, iend1=f1it->second.size(); i1<iend1; i1++)
                {
                    ...
                    if(pKF1 -> GetNLeft() != -1 && idx1 >= vKeysUn1.size()){
                        continue;
                    }
                    ...
                    cv::Mat d1 = pKF1->GetDescriptor(idx1);
                    if(d1.empty())
                        continue;
                    ...
                    for(size_t i2=0, iend2=f2it->second.size(); i2<iend2; i2++)
                    {
                        ...
                        if(pKF2 -> GetNLeft() != -1 && idx2 >= vKeysUn2.size()){
                            continue;
                        }
                        ...
                        cv::Mat d2 = pKF2->GetDescriptor(idx2);
                        if(d2.empty())
                            continue;
    ...
    int ORBmatcher::SearchByBoW(shared_ptr<KeyFrame> pKF1, shared_ptr<KeyFrame>  pKF2,
                                std::vector<shared_ptr<KeyFrame>> &vpMatchedCurrentKeyFrame,
                                std::vector<shared_ptr<MapPoint>> &vpMatchedCurrentMapPoint,
                                std::vector<shared_ptr<KeyFrame>> &vpMatchedLoopKeyFrame,
                                std::vector<shared_ptr<MapPoint>> &vpMatchedLoopMapPoint,
                                long unsigned int &nCurrentId)
    {
        const vector<cv::KeyPoint> &vKeysUn1 = pKF1->GetAllKeyUn();
        const DBoW2::FeatureVector &vFeatVec1 = pKF1->GetFeatureVector();
        const vector<shared_ptr<MapPoint>> &vpMapPoints1 = pKF1->GetMapPointMatches();
        ...
        vector<int> vpMatches12 = vector<int>(vpMapPoints1.size(), -1);
        vector<bool> vbMatched2(vpMapPoints2.size(),false);
        ...
        while(f1it != f1end && f2it != f2end)
        {
            if(f1it->first == f2it->first)
            {
                for(size_t i1=0, iend1=f1it->second.size(); i1<iend1; i1++)
                {
                    ...
                    if(pMP1->isBad())
                        continue;
                    if(pMP1->mnLoopPointForKF == nCurrentId)
                        continue;
                    ...
                    for(size_t i2=0, iend2=f2it->second.size(); i2<iend2; i2++)
                    {
                        ...
                        if(pMP2->mnLoopPointForKF == nCurrentId)
                            continue;
        ...
        for (int i = 0; i < HISTO_LENGTH; i++) {
            if (i == ind1 || i == ind2 || i == ind3) {
                for (size_t j = 0, jend = rotHist[i].size(); j < jend; j++) {
                    int idx1 = rotHist[i][j];
                    int idx2 = vpMatches12[idx1];
                    vpMapPoints1[idx1]->mnLoopPointForKF = nCurrentId;
                    vpMapPoints2[idx2]->mnLoopPointForKF = nCurrentId;
                    vpMatchedCurrentKeyFrame.push_back(pKF1);
                    vpMatchedCurrentMapPoint.push_back(vpMapPoints1[idx1]);
                    vpMatchedLoopKeyFrame.push_back(pKF2);
                    vpMatchedLoopMapPoint.push_back(vpMapPoints2[idx2]);
                    nmatches++;
                }
            }
        }
                                           ...
                                           vector<pair<size_t, size_t> > &vMatchedPairs, const bool bOnlyStereo, const bool bCoarse)
    {
        ...
        int nmatches=0;
        vector<bool> vbMatched2(pKF2->GetN(),false);
        vector<int> vMatches12(pKF1->GetN(),-1);
        ...
        while(f1it!=f1end && f2it!=f2end)
        {
            if(f1it->first == f2it->first)
            {
                for(size_t i1=0, iend1=f1it->second.size(); i1<iend1; i1++)
                {
                    ...
                    const bool bStereo1 = (!pKF1->mpCamera2 && pKF1->GetuRight(idx1)>=0);

                    if(bOnlyStereo)
                        if(!bStereo1)
                            continue;

                    const cv::KeyPoint &kp1 = pKF1->GetKeyPoint(idx1);
                    const bool bRight1 = pKF1 -> FromRightImage(idx1);
                    cv::Mat d1 = pKF1->GetDescriptor(idx1);
                    if(d1.empty())
                        continue;
                    ...
                    for(size_t i2=0, iend2=f2it->second.size(); i2<iend2; i2++)
                    {
                        ...
                        const bool bStereo2 = (!pKF2->mpCamera2 &&  pKF2->GetuRight(idx2)>=0);
                        ...
                        const cv::KeyPoint &kp2 = pKF2->GetKeyPoint(idx2);
                        const bool bRight2 = pKF2->FromRightImage(idx2);
                    ...
                    if(bestIdx2>=0)
                    {
                        const cv::KeyPoint &kp2 = pKF2->GetKeyPoint(bestIdx2);
    ...
    int ORBmatcher::Fuse(shared_ptr<KeyFrame> pKF, const vector<shared_ptr<MapPoint>> &vpMapPoints, const float th, const bool bRight)
    {
        ...
        for(int i=0; i<nMPs; i++)
        {
            ...
            {
                size_t idx = *vit;
                const cv::KeyPoint &kp = pKF->GetKeyPoint(idx);
                const int &kpLevel= kp.octave;

                if(kpLevel<nPredictedLevel-1 || kpLevel>nPredictedLevel)
                    continue;

                const float &kpr = pKF->GetuRight(idx);
                if(kpr>=0)
                {
    ...
    int ORBmatcher::SearchBySim3(shared_ptr<KeyFrame>  pKF1, shared_ptr<KeyFrame>  pKF2, std::vector<shared_ptr<MapPoint>> &vpMatches12, const Sophus::Sim3f &S12, const float th)
    {
        ...
        for(int i1=0; i1<N1; i1++)
        {
            ...
            for(vector<size_t>::const_iterator vit=vIndices.begin(), vend=vIndices.end(); vit!=vend; vit++)
            {
                ...
                const cv::KeyPoint &kp = pKF2->GetKeyUn(idx);
        ...
        for(int i2=0; i2<N2; i2++)
        {
            ...
            for(vector<size_t>::const_iterator vit=vIndices.begin(), vend=vIndices.end(); vit!=vend; vit++)
            {
                ...
                const cv::KeyPoint &kp = pKF1->GetKeyUn(idx);
            ...
            if(pMP)
            {
                if(!pMP->isBad() && !sAlreadyFound.count(pMP))
                {
                    ...
                    if(bestDist<=ORBdist)
                    {
                        ...
                        if(mbCheckOrientation)
                        {
                            float rot = pKF->GetKeyUn(i).angle-CurrentFrame.mvKeysUn[bestIdx2].angle;
                            if(rot<0.0)
                                rot+=360.0f;
                            int bin = round(rot*factor);
                            if(bin==HISTO_LENGTH)
