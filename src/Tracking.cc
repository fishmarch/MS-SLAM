...
void Tracking::UpdateLocalKeyFrames()
{
    ...
    shared_ptr<KeyFrame> pLocalLoopKF = shared_ptr<KeyFrame>();
    {
        unique_lock<mutex> lock(mMutexLoopKF);
        pLocalLoopKF = mpLoopKF;
    }
    if(pLocalLoopKF){
//        cout << "Local Loop KF: " << pLocalLoopKF->mnId << endl;
        shared_ptr<KeyFrame> pNewLoopKF;
        int max=0;
        const vector<shared_ptr<KeyFrame> > vNeighs = pLocalLoopKF->GetBestCovisibilityKeyFrames(20);
        for(vector<shared_ptr<KeyFrame> >::const_iterator itNeighKF=vNeighs.begin(), itEndNeighKF=vNeighs.end(); itNeighKF!=itEndNeighKF; itNeighKF++){
            ...
            if(!pNeighKF->isBad() && pNeighKF->mbSparsified){
                mvpLocalKeyFrames.push_back(pNeighKF);
                pNeighKF->mnTrackReferenceForFrame=mCurrentFrame.mnId;
                if(keyframeCounter.count(pNeighKF) && keyframeCounter[pNeighKF] > max){
                    max =keyframeCounter[pNeighKF];
                    pNewLoopKF = pNeighKF;
                }
            }
        }
        if(pNewLoopKF){
            ...
            unique_lock<mutex> lock(mMutexLoopKF);
            nnLostLoopCount++;
            if(nnLostLoopCount > 10)
                mpLoopKF.reset();
        }
    }

    int max=0;
    shared_ptr<KeyFrame>  pKFmax;
//    cout << mCurrentFrame.mnId << " : ";
    ...
    if(pCurrentMap->GetIniertialBA2()) {
        for (list<shared_ptr<KeyFrame> >::iterator lit = mlpKeyFramesToSparsification.begin(), lend = mlpKeyFramesToSparsification.end();
            ...
            if (pKFi->GetMap() != pCurrentMap || pKFi->isBad())
...
//        mpLocalMapper->RequestReset();
    ...
    mpLastKeyFrame.reset();
    mvIniMatches.clear();
    mlpKeyFramesToSparsification.clear();
    mbVelocity = false;
...
void Tracking::SetLoopKeyFrame(shared_ptr<ORB_SLAM3::KeyFrame> pKF) {
    unique_lock<mutex> lock(mMutexLoopKF);
    mpLoopKF = pKF;
    nnLostLoopCount = 0;
}
