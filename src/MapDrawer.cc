...
namespace ORB_SLAM3 {
    ...
    void MapDrawer::DrawMapPoints() {
        ...
        const vector<shared_ptr<MapPoint>> &vpSparsifiedMPs = pActiveMap->GetAllSparsifiedMapPoints();

        set<shared_ptr<MapPoint>> spSparsifiedMPs(vpSparsifiedMPs.begin(), vpSparsifiedMPs.end());
        ...
        for (size_t i = 0, iend = vpMPs.size(); i < iend; i++) {
            if (!vpMPs[i] || vpMPs[i]->isBad() || spSparsifiedMPs.count(vpMPs[i]))
                continue;
        ...
        for (size_t i = 0, iend = vpSparsifiedMPs.size(); i < iend; i++) {
            if (!vpSparsifiedMPs[i] || vpSparsifiedMPs[i]->isBad())
                continue;
            Eigen::Matrix<float, 3, 1> pos = vpSparsifiedMPs[i]->GetWorldPos();
            glVertex3f(pos(0), pos(1), pos(2));
        }
        glEnd();
    }
    ...
    void MapDrawer::DrawKeyFrames(const bool bDrawKF, const bool bDrawGraph, const bool bDrawInertialGraph,
        ...
        const vector<shared_ptr<KeyFrame> > vpSparsifiedKFs = pActiveMap->GetAllSparsifiedKeyFrames();
        set<shared_ptr<KeyFrame> > spSparsifiedKFs(vpSparsifiedKFs.begin(), vpSparsifiedKFs.end());
