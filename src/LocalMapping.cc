    ...
    while(1)
    {
        ...
        if(CheckNewKeyFrames() && !mbBadImu)
        {
            ...
            if(!mbInertial) {
                ...
                for (list<shared_ptr<KeyFrame> >::iterator lit = mlpKeyFramesToSparsification.begin(), lend = mlpKeyFramesToSparsification.end();
                     lit != lend;) {
...
void LocalMapping::CreateNewMapPoints()
{
    ...
    for(size_t i=0; i<vpNeighKFs.size(); i++)
    {
        ...
        for(int ikp=0; ikp<nmatches; ikp++)
        {
            ...
            const cv::KeyPoint &kp1 = mpCurrentKeyFrame->GetKeyPoint(idx1);

            const float kp1_ur=mpCurrentKeyFrame->GetuRight(idx1);
            bool bStereo1 = (!mpCurrentKeyFrame->mpCamera2 && kp1_ur>=0);
            const bool bRight1 = mpCurrentKeyFrame->FromRightImage(idx1);

            const cv::KeyPoint &kp2 = pKF2->GetKeyPoint(idx2);

            const float kp2_ur = pKF2->GetuRight(idx2);
            ...
            if(bStereo1)
                cosParallaxStereo1 = cos(2*atan2(mpCurrentKeyFrame->mb/2,mpCurrentKeyFrame->GetDepth(idx1)));
            else if(bStereo2)
                cosParallaxStereo2 = cos(2*atan2(pKF2->mb/2,pKF2->GetDepth(idx2)));
...
void LocalMapping::SearchInNeighbors()
{
    ...
    if(mpCurrentKeyFrame->GetNLeft() != -1)
        ...
        for(size_t i=0, iend=vpMapPoints.size(); i<iend; i++)
        {
            ...
            if(pMP)
            {
                if(!pMP->isBad())
                {
                    if(!mbMonocular)
                    {
                        if(pKF->GetDepth(i)>pKF->mThDepth || pKF->GetDepth(i)<0)
                            continue;
                    }
                    ...
                    {
                        const int &scaleLevel = pKF->GetScaleLevel(i);
                        ...
                        for(map<shared_ptr<KeyFrame> , tuple<int,int>>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
                        {
                            ...
                            int scaleLeveli = -1;
                            if(pKFi -> GetNLeft() == -1)
                                scaleLeveli = pKFi->GetKeyUn(leftIndex).octave;
                            else {
                                if (leftIndex != -1) {
                                    scaleLeveli = pKFi->GetKey(leftIndex).octave;
                                }
                                if (rightIndex != -1) {
                                    int rightLevel = pKFi->GetKeyRight(rightIndex - pKFi->GetNLeft()).octave;
