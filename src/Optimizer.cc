...
namespace ORB_SLAM3 {
    ...
    void Optimizer::BundleAdjustment(const vector<shared_ptr<KeyFrame> > &vpKFs, const vector<shared_ptr<MapPoint>> &vpMP,
        ...
        for (size_t i = 0; i < vpMP.size(); i++) {
            ...
            for (map<shared_ptr<KeyFrame> , tuple<int, int>>::const_iterator mit = observations.begin();
                ...
                const float kp_ur = pKF->GetuRight(leftIndex);
                if (leftIndex != -1 && kp_ur < 0) {
                    const cv::KeyPoint &kpUn = pKF->GetKeyUn(leftIndex);
                    if(kpUn.octave > 10)
                        continue;
                ...
                } else if (leftIndex != -1 && kp_ur >= 0) //Stereo observation
                {
                    const cv::KeyPoint &kpUn = pKF->GetKeyUn(leftIndex);
                    if(kpUn.octave > 10)
                        continue;
                ...
                if (pKF->mpCamera2) {
                    ...
                    if (rightIndex != -1) {
                        rightIndex -= pKF->GetNLeft();

                        Eigen::Matrix<double, 2, 1> obs;
                        cv::KeyPoint kp = pKF->GetKeyRight(rightIndex);
                        obs << kp.pt.x, kp.pt.y;
        ...
        for (size_t i = 0; i < vpMPs.size(); i++) {
            ...
            for (map<shared_ptr<KeyFrame> , tuple<int, int>>::const_iterator mit = observations.begin(), mend = observations.end();
                ...
                if (!pKFi->isBad()) {
                    const int leftIndex = get<0>(mit->second);
                    cv::KeyPoint kpUn;
                    const float kp_ur = pKFi->GetuRight(leftIndex);

                    if (leftIndex != -1 && kp_ur < 0) // Monocular observation
                    {
                        kpUn = pKFi->GetKeyUn(leftIndex);
                        if(kpUn.octave > 10)
                            continue;
                    ...
                    if (pKFi->mpCamera2) { // Monocular right observation
                        ...
                        if (rightIndex != -1) {
                            rightIndex -= pKFi->GetNLeft();

                            Eigen::Matrix<double, 2, 1> obs;
                            kpUn = pKFi->GetKeyRight(rightIndex);
                            obs << kpUn.pt.x, kpUn.pt.y;
        ...
        for (list<shared_ptr<MapPoint>>::iterator lit = lLocalMapPoints.begin(), lend = lLocalMapPoints.end();
            ...
            for (map<shared_ptr<KeyFrame>, tuple<int, int>>::const_iterator mit = observations.begin(), mend = observations.end();
                ...
                if (!pKFi->isBad() && pKFi->GetMap() == pCurrentMap) {
                    ...
                    // Monocular observation
                    if (leftIndex != -1 && kp_ur < 0) {
                        const cv::KeyPoint &kpUn = pKFi->GetKeyUn(leftIndex);
                        if(kpUn.octave > 10)
                            continue;
                    ...
                    if (pKFi->mpCamera2) {
                        ...
                        if (rightIndex != -1) {
                            ...
                            Eigen::Matrix<double, 2, 1> obs;
                            cv::KeyPoint kp = pKFi->GetKeyRight(rightIndex);
                            obs << kp.pt.x, kp.pt.y;
    ...
    void Optimizer::OptimizeEssentialGraph(Map *pMap, shared_ptr<KeyFrame>pLoopKF, shared_ptr<KeyFrame>pCurKF,
        ...
        for (map<shared_ptr<KeyFrame>, set<shared_ptr<KeyFrame>> >::const_iterator mit = LoopConnections.begin(), mend = LoopConnections.end();
            ...
            for (set<shared_ptr<KeyFrame>>::const_iterator sit = spConnections.begin(), send = spConnections.end();
                ...
                if ((nIDi != pCurKF->mnId || nIDj != pLoopKF->mnId) && pKF->GetWeight(*sit) < minFeat * ratio)
                    continue;
    ...
    int Optimizer::OptimizeSim3(shared_ptr<KeyFrame>pKF1, shared_ptr<KeyFrame>pKF2, vector<shared_ptr<MapPoint>> &vpMatches1, g2o::Sim3 &g2oS12,
        ...
        for (int i = 0; i < N; i++) {
            ...
            const cv::KeyPoint &kpUn1 = pKF1->GetKeyUn(i);
            if(kpUn1.octave > 10)
                continue;
    ...
    int Optimizer::OptimizeSim3(shared_ptr<KeyFrame>pKF1, shared_ptr<KeyFrame>pKF2, vector<shared_ptr<MapPoint>> &vpMatches1,
                                vector<shared_ptr<MapPoint>> &vpMatches2, g2o::Sim3 &g2oS12,
                                const float th2,
                                const bool bFixScale, Eigen::Matrix<double, 7, 7> &mAcumHessian,
                                const bool bAllPoints) {
        g2o::SparseOptimizer optimizer;
        ...
        const Eigen::Matrix3f R2w = pKF2->GetRotation();
        const Eigen::Vector3f t2w = pKF2->GetTranslation();
        const Eigen::Vector3f O1w = pKF1->GetCameraCenter();
        const Eigen::Vector3f O2w = pKF2->GetCameraCenter();
        ...
        for (int i = 0; i < N; i++) {
            shared_ptr<MapPoint> pMP1 = vpMatches1[i];
            shared_ptr<MapPoint> pMP2 = vpMatches2[i];
            ...
            // Set edge x1 = S12*X2
            Eigen::Matrix<double, 2, 1> obs1 = pCamera1->project(P3D1c).cast<double>();

            ORB_SLAM3::EdgeSim3ProjectXYZ *e12 = new ORB_SLAM3::EdgeSim3ProjectXYZ();
            e12->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(id2)));
            e12->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(0)));
            e12->setMeasurement(obs1);
            Eigen::Vector3f PO1 = P3D1w-O1w;
            const float dist1 = PO1.norm();
            int nPredictedLevel1 = pMP1->PredictScale(dist1, pKF1);
            const float &invSigmaSquare1 = pKF1->mvInvLevelSigma2[nPredictedLevel1];
            e12->setInformation(Eigen::Matrix2d::Identity() * invSigmaSquare1);
            ...
            // Set edge x2 = S21*X1
            Eigen::Matrix<double, 2, 1> obs2 = pCamera2->project(P3D2c).cast<double>();

            ORB_SLAM3::EdgeInverseSim3ProjectXYZ *e21 = new ORB_SLAM3::EdgeInverseSim3ProjectXYZ();
            e21->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(id1)));
            e21->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(0)));
            e21->setMeasurement(obs2);
            Eigen::Vector3f PO2 = P3D2w-O2w;
            const float dist2 = PO2.norm();
            int nPredictedLevel2 = pMP2->PredictScale(dist2, pKF2);
            float invSigmaSquare2 = pKF2->mvInvLevelSigma2[nPredictedLevel2];
            e21->setInformation(Eigen::Matrix2d::Identity() * invSigmaSquare2);
    ...
    Optimizer::LocalBundleAdjustment(shared_ptr<KeyFrame>pMainKF, vector<shared_ptr<KeyFrame>> vpAdjustKF, vector<shared_ptr<KeyFrame>> vpFixedKF,
        ...
        for (unsigned int i = 0; i < vpMPs.size(); ++i) {
            ...
            for (map<shared_ptr<KeyFrame>, tuple<int, int>>::const_iterator mit = observations.begin();
                ...
                const cv::KeyPoint &kpUn = pKF->GetKeyUn(get<0>(mit->second));
                if(kpUn.octave > 10)
                    continue;

                nEdges++;

                const float kp_ur = pKF->GetuRight(get<0>(mit->second));

                if (kp_ur < 0) //Monocular
                {
                    mpObsMPs[pMPi]++;
                    Eigen::Matrix<double, 2, 1> obs;
                    obs << kpUn.pt.x, kpUn.pt.y;
        ...
        for (unsigned int i = 0; i < vpMPs.size(); ++i) {
            ...
            for (map<shared_ptr<KeyFrame>, tuple<int, int>>::const_iterator mit = observations.begin();
                ...
                if (pKF->GetuRight(get<0>(mit->second)) < 0) //Monocular
                {
                    mpObsFinalKFs[pKF]++;
                } else // RGBD or Stereo
                {
    ...
    void Optimizer::MergeInertialBA(shared_ptr<KeyFrame>pCurrKF, shared_ptr<KeyFrame>pMergeKF, bool *pbStopFlag, Map *pMap,
        ...
        for (list<shared_ptr<MapPoint> >::iterator lit = lLocalMapPoints.begin(), lend = lLocalMapPoints.end();
            ...
            for (map<shared_ptr<KeyFrame>, tuple<int, int>>::const_iterator mit = observations.begin(), mend = observations.end();
                ...
                if (!pKFi->isBad()) {
                    const cv::KeyPoint &kpUn = pKFi->GetKeyUn(get<0>(mit->second));
                    if(kpUn.octave > 10)
                        continue;
                    const float kp_ur = pKFi->GetuRight(get<0>(mit->second));
                    if (kp_ur < 0) // Monocular observation
                    {
                        Eigen::Matrix<double, 2, 1> obs;
                        obs << kpUn.pt.x, kpUn.pt.y;
