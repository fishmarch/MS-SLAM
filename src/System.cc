    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    mpMapSaprsification = new MapSparsification(strSettingsFile, mpAtlas, mSensor == IMU_MONOCULAR || mSensor == IMU_STEREO || mSensor == IMU_RGBD);
    mptMapSparsification = new thread(&ORB_SLAM3::MapSparsification::Run, mpMapSaprsification);
...
void System::SaveTrajectoryEuRoC(const string &filename, bool scale)
{
    ...
    list<double>::iterator lT = mpTracker->mlFrameTimes.begin();
    list<bool>::iterator lbL = mpTracker->mlbLost.begin();
    std::cout << "  Sequence " << " has " << mlRelativeFramePoses.size() << " Frames" << std::endl;
    for(list<Sophus::SE3f>::iterator lit=mlRelativeFramePoses.begin(), lend=mlRelativeFramePoses.end();
        lit!=lend;lit++, lRit++, lT++, lbL++)
    {
        ...
        Sophus::SE3f Trw = pKF->GetPose()*Twb; // Tcp*Tpw*Twb0=Tcb0 where b0 is the new world reference
//        if (mSensor == IMU_MONOCULAR || mSensor == IMU_STEREO || mSensor==IMU_RGBD)
//        {
...
long unsigned int System::MapPointsInMap(){
    ...
    long unsigned int System::CompressedMapPointsInMap(){
        return mpAtlas->CompressedMapPointsInMap();
    }
