    ...
    pCamera1(pKF1->mpCamera), pCamera2(pKF2->mpCamera)
{
    ...
    for(int i1=0; i1<mN1; i1++)
    {
        if(vpMatched12[i1])
        {
            ...
            const cv::KeyPoint &kp1 = pKF1->GetKeyUn(indexKF1);
            const cv::KeyPoint &kp2 = pKFm->GetKeyUn(indexKF2);
    ...
    Sim3Solver::Sim3Solver(shared_ptr<KeyFrame> pKFCurr, shared_ptr<KeyFrame> pKFLoop,
                           const std::vector<shared_ptr<KeyFrame>> &vpMatchedCurrentKeyFrame,
                           const std::vector<shared_ptr<MapPoint>> &vpMatchedCurrentMapPoint,
                           const std::vector<shared_ptr<KeyFrame>> &vpMatchedLoopKeyFrame,
                           const std::vector<shared_ptr<MapPoint>> &vpMatchedLoopMapPoint,
                           const bool bFixScale):
            mnIterations(0), mnBestInliers(0), mbFixScale(bFixScale),
            pCamera1(pKFCurr->mpCamera), pCamera2(pKFLoop->mpCamera)
    {
        mpKF1 = pKFCurr;
        mpKF2 = pKFLoop;

        mN1 = vpMatchedCurrentMapPoint.size();
        ...
        Eigen::Matrix3f Rcw1 = pKFCurr->GetRotation();
        Eigen::Vector3f tcw1 = pKFCurr->GetTranslation();
        Eigen::Matrix3f Rcw2 = pKFLoop->GetRotation();
        Eigen::Vector3f tcw2 = pKFLoop->GetTranslation();
        ...
        for(int i1=0; i1<mN1; i1++)
        {
            ...
            shared_ptr<MapPoint> pMP1 = vpMatchedCurrentMapPoint[i1];
            shared_ptr<MapPoint> pMP2 = vpMatchedLoopMapPoint[i1];
            if (!pMP1 || !pMP2)
                continue;
            if (pMP1->isBad() || pMP2->isBad())
                continue;

            shared_ptr<KeyFrame>  pKF1 = vpMatchedCurrentKeyFrame[i1];
            shared_ptr<KeyFrame>  pKF2 = vpMatchedLoopKeyFrame[i1];
            if (!pKF1 || !pKF2)
                continue;
            if (pKF1->isBad() || pKF2->isBad())
                continue;
            ...
            const cv::KeyPoint &kp1 = pKF1->GetKeyUn(indexKF1);
            const cv::KeyPoint &kp2 = pKF2->GetKeyUn(indexKF2);
...
void Sim3Solver::ComputeSim3(Eigen::Matrix3f &P1, Eigen::Matrix3f &P2)
{
    ...
    vec = 2*ang*vec/(vec.norm() + 1e-12); //Angle-axis representation. quaternion angle is the half
